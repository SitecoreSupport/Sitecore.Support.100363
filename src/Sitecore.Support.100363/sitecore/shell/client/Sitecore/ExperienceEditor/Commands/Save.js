    ...
    execute: function (context) {
      ...
      if (postElements) {
        for (var i = 0; i < postElements.length; i++) {
		  ...
		  //fix for support case #468495
		  while (fields[postElements[i].id].indexOf("%26quot%3B") > -1) {
                fields[postElements[i].id] = fields[postElements[i].id].replace("%26quot%3B", "-,sc_dq,-");
            }
		  //end fix
        }
      }
      ...
      Sitecore.ExperienceEditor.PageEditorProxy.onSaving();
